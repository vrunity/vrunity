...
class ExtinguisherFormPage extends StatefulWidget {
  final String tagNumber;

  ExtinguisherFormPage({required this.tagNumber});

  @override
  _ExtinguisherFormPageState createState() => _ExtinguisherFormPageState();
}

class _ExtinguisherFormPageState extends State<ExtinguisherFormPage> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (result["status"] == "success") {
          ...
          // Notify user of error or no data
          ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> navigateToReviewPage() async {
    if (!_validateForm()) {
      return;
    }
    ...
    if (companyId == null || companyId.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Company ID is missing. Cannot proceed to review.")),
      );
      return;
    }
    ...
    final data = {
      ...
      "company_id": companyId, // Send company_id only for DB matching
    };
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => ReviewPage(
          ...
          appendUrl: "https://esheapp.in/esheapp_php/append_new_row.php",
        ),
      ),
    );
  }
                    ...
                    child: Column(
                      children: [
                        GestureDetector(
                          ...
                          child: Icon(
                            Icons.help_outline,
                            color: toggles[header] == "N/A" ? Colors.blue : Colors.grey,
                            size: 32,
                          ),
                        ),
                        SizedBox(height: 8), // Add spacing between the icon and label
                ...
                child: ElevatedButton(
                  ...
                  child: const Text(
                    ...
                    style: TextStyle(
                      fontSize: 16,
                      fontWeight: FontWeight.bold,
                      color: Colors.white, // White text for contrast with gradient
                    ),
                  ),
                ),
              ),
